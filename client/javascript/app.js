$(document).ready(function () {
  ...
  const stateAbbreviations = [
    ...
    'WI',
    'WY',
  ]; // 50 united states plus district of columbia
  ...
  function populateSelectDialog(dialogId, inputArr) {
    for (item of inputArr) {
      $(`#${dialogId}`).append(`<div class='col-sm-4'>
    ...
    saveDialogSelection('thread-save', 'thread-select-options');
    saveDialogSelection('stitch-save', 'stitch-select-options');
  }

  function saveDialogSelection(btnId, dialogId) {
    dialog = $(`#${dialogId}`);
  ...
  function populateSelectBox(id, inputArr) {
    for (item of inputArr) {
      $(`#${id}`).append($('<option></option>').attr('value', item).text(item));
    }
  }
